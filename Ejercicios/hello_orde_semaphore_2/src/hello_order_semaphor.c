...
int main(int argc, char* argv[]) {
  ...
  if (shared_data) {
    ...
    for (uint64_t thread_number = 0; thread_number < shared_data->thread_count
      ...
      //------------------------------------------------------------------------
      // EJercicio 14, ver que pasa si pongo la posicion 0 en 1 y 
      // los demas en 0
      if (thread_number == 0) {
        sem_init(&shared_data->can_greet[thread_number], /*pshared*/ 0
        , /*value*/ 1);
      } else {
...
int create_threads(shared_data_t* shared_data) {
  ...
  if (threads && private_data) {
    ...
    //-----------------------------------------------------------------------------
    // Parte 2 del ejercicio 14, quitar la primera parte y poner esta para ver el
    // comportamiento
      // error = sem_init(&shared_data->can_greet[thread_number], /*pshared*/ 0
        // , /*value*/ 0);
    //-----------------------------------------------------------------------------    
