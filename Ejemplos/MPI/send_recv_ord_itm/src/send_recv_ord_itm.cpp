...
void greet(int process_number, int process_count
  ...
  if (process_number != 0) {
    ...
    for (int source = 1; source < process_count; ++source) {
      ...
      // que envia el mensaje por eso es message[0], esto hace que imprima
      // en orden ya que source hace que no lleguen todos de manera
      // indeterministica, para verlo mejor, esto es como una barrera,
      // no va a avanzar hasta que llegue el proceso que diga source
      // se puede poner any_source si se quiere dejar el indeterminismo
